...
const ListItemWrapper = styled(Stack)<PreviewStyleProps>`
  --bg-selected: ${(props) =>
    props.$currentScheme === "light" ? "#EFF1F2" : "#242B31"};
  --fg-selected: ${(props) =>
    props.$currentScheme === "light" ? "#242B31" : "#AFB7BD"};
  --hover-bg: ${(props) =>
    props.$currentScheme === "light" ? "#FCFCFC" : "#5F727F"};

  padding: 0.9rem 0rem 0.9rem 0.25rem;
...
const PublishIconContainer = styled(Text)<PreviewStyleProps>`
  --published: ${(props) =>
    props.$currentScheme === "light" ? "#3BE086" : "#3BE086"};
...
const EditIconContainer = styled(Text)<PreviewStyleProps>`
  --edited: ${(props) =>
    props.$currentScheme === "light" ? "5F727F" : "#AFB7BD"};
...
const ItemIcon = ({ item }: { item: TreeNode }) => {
  ...
  if (item._type === "folder") {
    return createElement(
      item.slug && folders?.[item.slug]?.icon
        ? folders[item.slug].icon!
        : FolderIcon,
      {
