...
export function buildTree(list: Page[]): Tree {
  ...
  for (const item of list) {
    ...
    const segments =
      item.slug === "/" ? [""] : item.slug?.split("/").filter(Boolean);
    ...
    segments?.forEach((segment, index) => {
      ...
      // e.g. slug: "lp/about", segment: "about", currentFolder: "lp/"
      if (!currentFolder[segment]) {
        currentFolder[segment] = node;
      }
      // e.g. slug: "lp", segment: "lp", currentFolder: root = {lp: {type: "folder"}}
      else if (!isFolder && currentFolder[segment]._type === "folder") {
        currentFolder[segment].children[""] = node;
      }
      // e.g. slug: "lp", segment: "lp", currentFolder: root = {lp: {type: "page"}}
      else if (isFolder && currentFolder[segment]._type !== "folder") {
